...
We run 16 different w1,w2 pairs to collect data. Through our observation, $ w_1=1.25 $ has advantages of number of node expanded and very good suboptimal path length,but the run time is relative highest among all 16 pairs.  Through many testing cases, Sequential A* search should be generally slower than regular A* search since it runs multiple heuristic search. But after we increased $w_1$ to 2.5, the run time becomes shorter than A* search. We can say between 2 and 2.5, there is a threshold for Sequential A* search to run faster than A* search. Also we observe $ w_1=2 $ has an medium performance on every aspect. Until here, we can make conclusion about, $ w_1 $ affects all these factors more than $w_2$,but with slightly twisting the value of $w_2$,we can have a slightly better run time performance while other stats are same,but this does not mean by infinite increasing $ w_2 $,we will get better run time performance. Checkout example with $ (w_1=1.25,w_2=1.25;w_1=1.25,w_2=2;w_1=1.25,w_2=3) $
